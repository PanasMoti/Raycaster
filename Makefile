...
wbuild:
	$(CC) $(SOURCES) $(IMPORTS) $(COMPILER_FLAGS) -o3 $(WIN_LINKER_FLAGS) -o $(BASE_BUILD_DIR)/$(EXE_NAME).exe
wrun:
	$(BASE_BUILD_DIR)/$(EXE_NAME).exe

wdebug:
	$(CC) $(SOURCES) $(IMPORTS) $(COMPILER_FLAGS) -o0 $(WIN_LINKER_FLAGS) -o $(BASE_BUILD_DIR)/$(DEBUG_DIR)/$(EXE_NAME).exe
	$(BASE_BUILD_DIR)/$(DEBUG_DIR)/$(EXE_NAME).exe

wrelease:
	$(CC) $(SOURCES) $(IMPORTS) $(COMPILER_FLAGS) -o3 $(WIN_LINKER_FLAGS) -o $(BASE_BUILD_DIR)/$(RELEASE_DIR)/$(EXE_NAME).exe
	$(BASE_BUILD_DIR)/$(RELEASE_DIR)/$(EXE_NAME).exe
