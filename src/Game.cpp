...
void Game::MainLoop() {
    while(!renderWindow->ShouldClose) {
        ...
        while(SDL_PollEvent(&sdlEvent)) {
            if(sdlEvent.type == SDL_QUIT) renderWindow->ShouldClose = true;
            if(sdlEvent.type == SDL_KEYDOWN) this->KeyDown(sdlEvent);
...
//             texWidth = ceil_test->getWidth();
//             texHeight = ceil_test->getHeight();
//             tx = (int)(texWidth * (floorP.x - cellX)) & (texWidth - 1);
//             ty = (int)(texHeight * (floorP.y - cellY)) & (texHeight - 1);
//             color = (*ceil_test)[texWidth*ty+tx];
//             renderWindow->set_pixel(x,screenHeight-1-y,color);
        }
    }
}
